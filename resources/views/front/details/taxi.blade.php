...
<div class="page-banner-2 ">
    <div class="container">
        <span>${{ number_format($taxi->from, 2, '.', ' ') }}</span>
        <h1>{{ $taxi->title }}</h1>
        <p>{{ $taxi->sub_title }}</p>
        <a href="#" data-bs-toggle="modal" data-bs-target="#taxiModal">Order Now</a>
    </div>
</div>
<!-- Page banner end -->
...
<main class="details-page">
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <div class="details">
                    @if (json_decode($taxi->features)[0]->name)
                    <div class="data-item features">
                        <h4>Detail & Features</h4>
                        
                        <ul>
                            @foreach (json_decode($taxi->features) as $f)
                            <li>
                                <span>{{ $f->name }}:</span>
                                <span>{{ $f->value }}</span>
                            </li>
            ...
            <div class="col-lg-4">
                <div class="sidebar">
                    ...
                    <div class="related-items">
                        <h4>Related Taxis</h4>
                        <ul>
                            @foreach ($randTaxi as $rand_i)
                            <li>
                                <a href="{{ route('taxi.single', $rand_i->slug) }}">
                                    <img src="{{ asset($rand_i->image) }}" alt="{{ $rand_i->title }}">
                                </a>
                                <div class="content">
                                    <h5><a href="{{ route('taxi.single', $rand_i->slug) }}">{{ $rand_i->title }}</a></h5>
        ...
        <div class="modal-body mb-3">
            <form action="{{ route('order.taxi') }}" method="POST">
                ...
                <div class="form-content pt-0">
                    <h4 id="taxiModalTitle">{{ $taxi->title }}</h4>
                    <input id="taxiItemId" type="hidden" name="taxi_id" value="{{ $taxi->id }}">
