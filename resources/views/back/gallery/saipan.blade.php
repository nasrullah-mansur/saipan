    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="dataForm" class="form" action="{{ route('admin.gallery.saipan.create') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="form-body">
                      <div class="row">
                        ...
                        <div class="col-lg-12">
                            <fieldset class="form-group">
                              <label for="taxi">Select Saipan Item</label>
                              <select name="saipan_id" class="form-control" id="taxi">
                                @foreach (App\Models\Admin\Saipan::all() as $saipan)
...
<script>
        ...
        e.preventDefault();
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        method: "GET",
                        url: '{{ route('admin.gallery.saipan.delete') }}',
                        data: {
    ...
    $('.table-area').on('click', '.edit-btn', function(e){
        ...
        let taxi_id = $(this).attr("data-saipan_id");
        console.log(taxi_id);
        $('#addModal form')[0].reset();
        $('#addModal').modal('show');
        $('#dataForm').attr('action', "{{ route('admin.gallery.saipan.update') }}");
        $('[name="alt"]').val(alt);
        $('[name="data_id"]').val(editDataId);
        $('[name="saipan_id"]').val(taxi_id);
        $('.preview_img').show();
    ...
    $('#addItemBtn').on('click', function(e) {
        ...
        $('#addModal form')[0].reset();
        $('.preview_img').hide();
        $('#dataForm').attr('action', "{{ route('admin.gallery.saipan.create') }}");
    })
