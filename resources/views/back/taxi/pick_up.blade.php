            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title mb-2">Pick up location list</h4>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="addModalLabel1">Pick up location</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="dataForm" class="form" action="{{ route('admin.pick.up.store') }}" method="POST">
                    @csrf
                    <div class="form-body">
                      <div class="row">
                        <div class="col-12">
...
<script>
        ...
        e.preventDefault();
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        method: "GET",
                        url: '{{ route('admin.pick.up.delete') }}',
                        data: {
    ...
    $('.table-area').on('click', '.edit-btn', function(e){
        ...
        $('#addModal').modal('show');
        $('#dataForm').attr('action', "{{ route('admin.pick.up.update') }}");
        $.ajax({
            method: "GET",
            url: '{{ route('admin.pick.up.edit') }}',
            data: {
                'id': editDataId,
            },
            success: function(response){
    ...
    $('#addItemBtn').on('click', function(e) {
        e.preventDefault();
        $('#addModal form')[0].reset();
        $('#dataForm').attr('action', "{{ route('admin.pick.up.store') }}");
    })
